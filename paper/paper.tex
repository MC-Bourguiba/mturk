...
\begin{figure}
  \centering
  %   \includegraphics[width=0.5\textwidth]{images/fill_me}
  \missingfigure[figwidth=6cm]{System block diagram}
...
\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{images/user_interface}
  \caption{Web interface for the game.}
...
%   \node [block, right of=sum, node distance=3cm] (system) {Network};

%   \node [output, right of=system] (output) {};
%   % \node [block, below of=system] (measurements) {Measurements};

%   % \draw [draw,->] (input) -- node {$\bar x(t)$} (sum);
%   \draw [->] (sum) -- node {$\bar x(t)$} (system);
%   % \draw [->] (y) |- (measurements);
%   % \draw [->] (measurements) -| node[pos=0.99] {$-$}
%   % node [near end] {$y_m$} (sum);

%   \node at (0,-1) (input1) {$i_1$};
%   \node[block, above left = of system, anchor=west] at (2,-1) (block1) {$p_1$};
%   \draw[->] (block1.east) -- (sum);
%   \draw[->] (system) -- (block1);

%   \node at (0,-2) (input2) {$i_2$};
%   \node[block, left of=system, anchor=west] at (2,-2) (block2) {$p_2$};
%   \draw[->] (block2.east) -- (sum);
%   \draw[->] (system) -- (block2);

%   \node at (0,-3) (input3) {$i_3$};
%   \node[block, below left = of system, anchor=west] at (2,-3) (block3) {$p_3$};
%   \draw[->] (block3.east) -- (sum);
%   \draw[->] (system) -- (block3);

%   % % Draw blocks, inputs and outputs
%   % \foreach \y in {1,2,3} {
%   %   \node at (0,-\y) (input\y) {$i_\y$};
%   %   \node[block, left of=system, above of =system] at (2,-\y) (block\y) {$p_\y$};
%   %   \draw[->] (input\y) -- (block\y);
%   %   \draw[->] (block\y.east) -- (sum);
%   %   \draw[->] (system) -- (block\y);
%   % }
...
At the beginning of the game, there is a clear exploration phase where players tend make aggressive assignments with their flow distribution. Towards the end of the game, the players consolidate their position and tend to make more conservative flow distribution assignments, as seen in Figure~\ref{fig:player_behavior} as an example.
...
We can estimate the learning rate of each player with the update in Eq~\ref{eq:estimation_eta}. We obtained these sequence of learning rates for each player and plotted the moving average of length 3 for better visualization in Figure~\ref{fig:moving_average_learning_rate}.

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{images/moving_average_learning_rate}
  \caption{Moving average learning rate.}
  \label{fig:moving_average_learning_rate}
\end{figure}

With these estimated sequences of learning rates, we can predict $x_{t+i+1}$ by using the prediction and propagation in \eqref{eq:estimation_step} and \eqref{eq:propagation}. To obtain $\eta^{(t+i)}_k$, we propose three methods. First, we assume $\eta^{(t+i)}_k = \eta^{(t+i-1)}_k$, where $\eta^{t+i-1}_k$ is the estimated learning rate in the previous turn as a baseline method for evaluation. Second, we assume $\eta^{(t+i)}_k$ to be the moving average of the previous estimated etas in the sequence. Finally, we perform linear regression on the estimated $(\eta_k^{(1)}, \dots, \eta_k^{(t-1)})$ to calculate $\eta^{(t+i)}_k$.

We also compare these methods of computing $\eta^{(t+1)}$ with a parameterized learning rate proposal in Section~\ref{sec:estimation}. To obtain the parameterized sequence of $(\eta_k^{(1)}, \eta_k^{(2)}, \dots)$, we perform solve the non-convex optimization problem in \eqref{eq:estimation_alpha_eta0} for each player $k$. Then we use the optimal $\alpha_k, \eta^{(0)}_{k}$ to obtain the learning rate $\eta^{(t+i)}_k$ for our prediction and propagation. These sequences of learning rates can be seen in Figure~\ref{fig:sequence_rate}.

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{images/sequence_rate}
  \caption{Estimated parameterized learning rate.}
  \label{fig:sequence_rate}
\end{figure}

We evaluate each method by computing the average Bregman divergence per player per turn. From the results, the farther ahead we look in our prediction, the average Bregman divergence tend to increase for all methods. As the interval of prediction increases, the modeling errors propagate and the quality of our predictions degrade, which is reflected in Figure~\ref{fig:predictions_divergence}.

\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{images/predictions_divergence}
  \caption{Average Bregman divergence per player and per iteration.}
  \label{fig:predictions_divergence}
\end{figure}

We show an example of the two-step ahead flow distribution predictions made by the parameterized method in Figure~\ref{fig:parameterized} for all of the paths of one player. The flow predictions for each path are generally close to the actual flow distribution from the player. In cases where the player makes an extreme move, the flow predictions tend to underestimate the magnitude of the change in the flow distribution.
